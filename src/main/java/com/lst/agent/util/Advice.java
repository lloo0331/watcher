...
//            ON_METHOD_END = Advice.class.getMethod("onMethodEnd", Object.class);
    ...
    public static void enterWith(Map<String, Object> context){};

    public static void exitWith(Map<String, Object> context){

        Long end = (Long) context.get(STOPPED);
        Long start = (Long) context.get(STARTED);
        long ms = end-start;
        if(ms>0){
            System.out.println(context.get(CLASS)+"."+context.get(METHOD)+":"+(end-start));
        }
